...
char MenuNuevoPuntaje( Objeto **p_Ini, ALLEGRO_DISPLAY **p_display, int Puntos, const char *Titulo ){	
	...
	if( Retornar == CONTINUAR ){
		if( GuardarOrdenadoEnArchivo( Nombre, Puntos, "Archivos/Puntajes.txt" ) == ERROR ) Retornar = ERROR;
		Retornar = MenuPuntajes( &Eventos_Nuevo_Puntaje );
	}
