...
void RanaSaltaArriba( Objeto **p_ObjetoFrog, ALLEGRO_BITMAP **VecFrog, char *p_ContadorSprites ){
	...
	if( ObjetoFrog->Pos_y == ( UPPER_OFFSET + BOUNCE_Y - 2 ) ){			//CONDICION DEL SALTO DE LLEGADA
		if( ObjetoFrog->Pos_x > ( X_POS_1 - MARGEN_SALTO_FINAL ) && ObjetoFrog->Pos_x < ( X_POS_1 + MARGEN_SALTO_FINAL ) ) {
			ObjetoFrog->Pos_x = X_POS_1; 
			ObjetoFrog->Pos_y = UPPER_OFFSET;
			ObjetoFrog->Imagen = VecFrog[0];
		}
		if( ObjetoFrog->Pos_x > ( X_POS_2 - MARGEN_SALTO_FINAL ) && ObjetoFrog->Pos_x < ( X_POS_2 + MARGEN_SALTO_FINAL ) ) {
			ObjetoFrog->Pos_x = X_POS_2; 
			ObjetoFrog->Pos_y = UPPER_OFFSET;
			ObjetoFrog->Imagen = VecFrog[0];
		}
		if( ObjetoFrog->Pos_x > ( X_POS_3 - MARGEN_SALTO_FINAL ) && ObjetoFrog->Pos_x < ( X_POS_3 + MARGEN_SALTO_FINAL ) ) {
			ObjetoFrog->Pos_x = X_POS_3;
			ObjetoFrog->Pos_y = UPPER_OFFSET;
			ObjetoFrog->Imagen = VecFrog[0];
		}
		if( ObjetoFrog->Pos_x > ( X_POS_4 - MARGEN_SALTO_FINAL ) && ObjetoFrog->Pos_x < ( X_POS_4 + MARGEN_SALTO_FINAL ) ) {
			ObjetoFrog->Pos_x = X_POS_4;
			ObjetoFrog->Pos_y = UPPER_OFFSET;
			ObjetoFrog->Imagen = VecFrog[0];
		}
		if( ObjetoFrog->Pos_x > ( X_POS_5 - MARGEN_SALTO_FINAL ) && ObjetoFrog->Pos_x < ( X_POS_5 + MARGEN_SALTO_FINAL ) ) {
